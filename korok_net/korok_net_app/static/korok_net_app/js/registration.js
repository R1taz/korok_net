document.addEventListener("DOMContentLoaded", () => {
    ...
    username?.addEventListener("input", () => validateUsernameField(username));
    username?.addEventListener("blur", () => validateUsernameField(username, true));
...
function validateFullName(input, errorId) {
    ...
    if (value && parts.length > 3) {
        showError(errorId, "Укажите не более трех слов: фамилия, имя и отчество.");
        input?.classList.add("error");
        return false;
    }
...
function validateUsernameField(input, shouldCheckUnique = false) {
    const isFormatValid = validateLatinAndNumbers(input, "username_error");
    if (!isFormatValid) {
        return false;
    }

    const isLengthValid = checkUsernameLength(input?.value || "");
    if (!isLengthValid) {
        return false;
    }

    if (shouldCheckUnique) {
        checkUsernameUnique(input?.value || "");
    }
