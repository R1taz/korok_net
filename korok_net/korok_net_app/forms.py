...
class RegistrationUserForm(StyledFormMixin, UserCreationForm):
    username = forms.CharField(
        ...
        widget=forms.TextInput(
            attrs={
                "placeholder": "Например, northwave",
...
class AuthorizationUserForm(StyledFormMixin, AuthenticationForm):
    username = forms.CharField(
        label="Логин",
        widget=forms.TextInput(
            attrs={"placeholder": "Логин вашего профиля", "autocomplete": "username"}
        ),
    )
    password = forms.CharField(
        label="Пароль",
        widget=forms.PasswordInput(
            attrs={"placeholder": "Введите текущий пароль", "autocomplete": "current-password"}
        ),
    )
...
class ReviewForm(StyledFormMixin, forms.ModelForm):
    ...
    text = forms.CharField(
        ...
        widget=forms.Textarea(
            attrs={
                "rows": 4,
                "placeholder": "Поделитесь впечатлением о программе, материалах и процессе обучения",
            }
        ),
        min_length=10,
    )
...
class AdminApplicationFilterForm(StyledFormMixin, forms.Form):
    q = forms.CharField(
        label="Поиск",
        required=False,
        widget=forms.TextInput(attrs={"placeholder": "Имя, курс или номер телефона"}),
    )
